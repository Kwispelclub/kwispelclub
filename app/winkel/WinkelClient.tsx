...
export default function WinkelPage() {
  ...
  const addToCart = (p: any) => {
    try {
      ...
      const maxVoorraad = p.voorraad ?? p.stock
      if (existing >= 0) {
        if (maxVoorraad !== null && maxVoorraad !== undefined && cart[existing].aantal >= maxVoorraad) {
          alert(`Je hebt het maximale aantal van "${p.name}" in je winkelwagen.`)
          return
        }
                ...
                <div key={p.id} className="product-card" style={{cursor:'pointer'}} onClick={() => { setSelectedProduct(p); setModalAantal(1); setModalAdded(false) }}>
                  ...
                  <div className="product-body">
                    ...
                    <button
                      ...
                      onClick={e => { e.stopPropagation(); if ((p.voorraad ?? p.stock) === 0) return; addToCart(p) }}
          ...
          <div className="modal" onClick={e => e.stopPropagation()} style={{position:'relative'}}>
            ...
            <div className="modal-body">
              ...
              <div className="aantal-ctrl">
                ...
                <button className="aantal-btn"
                  onClick={() => setModalAantal(a => {
                    const max = selectedProduct.voorraad ?? selectedProduct.stock
                    return (max !== null && max !== undefined) ? Math.min(a + 1, max) : a + 1
                  })}
                  disabled={(() => { const max = selectedProduct.voorraad ?? selectedProduct.stock; return max !== null && max !== undefined && modalAantal >= max })()}
                >+</button>
                <span style={{fontSize:13,color:'var(--text-light)',fontWeight:600}}>= €{(parseFloat(selectedProduct.price) * modalAantal).toFixed(2)}</span>
                {(() => { const max = selectedProduct.voorraad ?? selectedProduct.stock; return max !== null && max !== undefined && modalAantal >= max ? <span style={{fontSize:11,color:'var(--red)',fontWeight:700,marginLeft:8}}>Max bereikt</span> : null })()}
              </div>
