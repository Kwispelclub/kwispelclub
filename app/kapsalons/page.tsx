...
export default function KapsalonsPage() {
  ...
  // Pre-fill registration with logged-in user data
  useEffect(() => {
  ...
  const handleRegister = async () => {
    ...
    const { error } = await supabase.from('kapsalons').insert({
      ...
      actief: false, // wacht op admin goedkeuring
      geverifieerd: false,
    })

    if (error) {
      // Als niet ingelogd of RLS error, sla op als aanvraag
      setRegError('Er ging iets mis. Stuur een e-mail naar info@kwispelclub.be')
    } else {
      setRegDone(true)
      // Update user role als kapsalon
      if (user) {
  ...
  return (
    <>
      ...
      <section className="section" id="salons">
        ...
        <div className="demo-notice fade-up">⚠️ <span>Voorbeeldsalons</span> — Heb je een salon? <a href="#register">Registreer je alvast →</a></div>
          ...
          <div className="reg-form">
            {regDone ? (
              ...
              <>
                ...
                <button className="btn btn-primary" style={{width:'100%',justifyContent:'center',marginTop:8,opacity:regLoading?.6:1}} onClick={handleRegister} disabled={regLoading}>
        ...
        <div className="modal-overlay" onClick={e => e.target===e.currentTarget&&closeModal()}>
          <div className="modal">
            <div className="modal-header">
              <div><h2>{modal.salon.label}</h2><p>📍 {modal.salon.location}</p></div>
              ...
              {step===5&&selSvc&&selDate&&(
                <div className="success-state">
                  ...
                  <div className="success-details">
                    <strong>📍 {modal.salon.label}</strong><br/>{modal.salon.location}<br/><br/>
