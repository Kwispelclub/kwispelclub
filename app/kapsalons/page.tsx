...
const SALONS = [
  { id:1, region:'limburg', name:'happy paws grooming bree', label:'Happy Paws Grooming', rating:4.9, reviews:87, location:'Bree, Limburg', tags:['Alle rassen','Hand-stripping','Puppypakket'], price:30, badge:'top', img:'https://images.unsplash.com/photo-1527526029430-319f10814151?w=500&q=80' },
  { id:2, region:'limburg', name:'de gouden schaar hasselt', label:'De Gouden Schaar', rating:4.7, reviews:42, location:'Hasselt, Limburg', tags:['Kleine rassen','Spa-behandeling'], price:25, badge:'new', img:'https://images.unsplash.com/photo-1581888227599-779811939961?w=500&q=80' },
  { id:3, region:'antwerpen', name:'woef wellness antwerpen', label:'Woef Wellness', rating:4.8, reviews:156, location:'Antwerpen', tags:['Premium salon','Grote rassen','Ophaalservice'], price:40, badge:'top', img:'https://images.unsplash.com/photo-1548199973-03cce0bbc87b?w=500&q=80' },
  { id:4, region:'brabant', name:'trim en style leuven', label:'Trim & Style', rating:4.6, reviews:68, location:'Leuven, Vlaams-Brabant', tags:['Show-trimmen','Wedstrijdvoorbereiding'], price:35, badge:'promo', img:'https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=500&q=80' },
  { id:5, region:'oost-vl', name:'knip en kwispel gent', label:'Knip & Kwispel', rating:4.9, reviews:203, location:'Gent, Oost-Vlaanderen', tags:['Biologische producten','Angst-vrij trimmen'], price:32, badge:'', img:'https://images.unsplash.com/photo-1518717758536-85ae29035b6d?w=500&q=80' },
  { id:6, region:'nl', name:'dierenkappers de luxe maastricht', label:'Dierenkappers de Luxe', rating:4.8, reviews:134, location:'Maastricht, Nederland', tags:['Hond & Kat','Creatief trimmen','Senior dieren'], price:28, badge:'top', img:'https://images.unsplash.com/photo-1574158622682-e40e69881006?w=500&q=80' },
]
...
export default function KapsalonsPage() {
  ...
  const [modal, setModal] = useState<{salon:typeof SALONS[0]}|null>(null)
  ...
  const filtered = SALONS.filter(s => (region==='all'||s.region===region) && s.name.includes(search.toLowerCase()))

  const openModal = (salon: typeof SALONS[0]) => {
    setModal({salon}); setStep(1); setSelSvc(null); setSelDate(null); setSelTime(''); setPetName(''); setPetBreed(''); setOwnerName(''); setOwnerPhone(''); setOwnerEmail(''); setNotes('')
  ...
  const CSS = `
    ...
    *{margin:0;padding:0;box-sizing:border-box}body{font-family:'Nunito',sans-serif;background:var(--cream);color:var(--text-dark);overflow-x:hidden;-webkit-font-smoothing:antialiased}h1,h2,h3,h4{font-family:'Fredoka',sans-serif}
    ...
    .demo-notice{background:var(--orange-pale);border:2px dashed var(--orange-main);border-radius:12px;padding:14px 20px;text-align:center;font-size:13px;font-weight:600;color:var(--brown);margin-bottom:24px}.demo-notice span{color:var(--orange-main)}.demo-notice a{color:var(--orange-main);font-weight:700}
    ...
    .salons-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:24px}.salon-card{background:var(--white);border-radius:20px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.06);transition:all .3s;border:2px solid transparent}.salon-card:hover{transform:translateY(-6px);box-shadow:0 8px 40px rgba(0,0,0,.12);border-color:var(--green-pale)}
    ...
    .salon-bdg{position:absolute;top:14px;left:14px;display:flex;gap:6px}.salon-badge{padding:4px 12px;border-radius:50px;font-size:11px;font-weight:700;color:white}.btop{background:var(--orange-main)}.bnew{background:var(--green-main)}.bpromo{background:var(--red)}
    ...
    @media(max-width:768px){.nav-links{display:none}.salons-grid{grid-template-columns:1fr}.hero-content{padding:36px 24px}}
  ...
  return (
    <>
      ...
      <section className="section">
        <div className="services-box">
          ...
          <div className="services-grid">
            {[['✂️','Volledig Trimmen','Wassen, knippen, drogen en stylen op maat van het ras','Vanaf €35'],['🛁','Wassen & Drogen','Professioneel bad met speciale shampoo en föhnen','Vanaf €20'],['💅','Nagels Knippen','Veilig en stress-vrij nagelverzorging door experts','Vanaf €10'],['👂','Oren & Tanden','Oorcontrole, reiniging en tandenpoetsen','Vanaf €15']].map(([icon,title,desc,price]) => (
      ...
      <section className="section" id="salons">
        <div className="section-header"><h2>Kapsalons bij jou in de buurt 📍</h2><p>Geverifieerde salons met echte klantbeoordelingen</p></div>
        <div className="demo-notice">⚠️ <span>Voorbeeldsalons</span> — Deze salonprofielen zijn fictief ter illustratie. Geen echte bedrijven. Heb je een salon? <a href="#register">Registreer je alvast →</a></div>
            ...
            <div key={s.id} className="salon-card">
              <div className="salon-cover">
                ...
                <div className="salon-bdg">
                  {s.badge==='top' && <span className="salon-badge btop">⭐ TOP</span>}
                  {s.badge==='new' && <span className="salon-badge bnew">NIEUW</span>}
                  {s.badge==='promo' && <span className="salon-badge bpromo">-15% INTRO</span>}
                </div>
              ...
              <div className="salon-info">
                <div className="salon-head"><div className="salon-name">{s.label}</div><div className="salon-rating">⭐ {s.rating} <span>({s.reviews})</span></div></div>
                <div className="salon-loc">📍 {s.location}</div>
                <div className="salon-tags">{s.tags.map((t,i) => <span key={t} className={`tag ${i===0?'sp':''}`}>{t}</span>)}</div>
                <div className="salon-details">
                  <div className="salon-price">Vanaf <strong>€{s.price}</strong></div>
      ...
      <section className="section" id="register">
        <div className="register-section">
          ...
          <div className="reg-form">
            {regDone ? (
              <div className="reg-success"><div className="si">✅</div><h3 style={{color:'white',marginBottom:8}}>Aanvraag verstuurd!</h3><p style={{opacity:.8,fontSize:14}}>We nemen zo snel mogelijk contact met je op.</p></div>
            ) : (
              <>
                <h3>Registreer je Salon</h3>
                <div className="fg"><label>Salonnaam</label><input placeholder="Bijv. Happy Paws Grooming" value={regName} onChange={e => setRegName(e.target.value)} /></div>
                <div className="fg"><label>Locatie</label><input placeholder="Stad, provincie" value={regLoc} onChange={e => setRegLoc(e.target.value)} /></div>
                ...
                <div className="fg"><label>Type Salon</label><select><option>Trimsalon (alle rassen)</option><option>Gespecialiseerd (kleine rassen)</option><option>Gespecialiseerd (grote rassen)</option><option>Mobiele trimmer</option><option>Premium / Spa salon</option></select></div>
                <button className="btn btn-primary" style={{width:'100%',justifyContent:'center',marginTop:8}} onClick={() => setRegDone(true)}>Gratis registreren →</button>
              </>
            )}
          </div>
        </div>
      ...
      <footer>
        <div className="footer-inner">
          ...
          <div className="footer-links"><a href="/">Home</a><a href="/#shop">Shop</a><a href="/puppy-training">Puppy Training</a><a href="/kapsalons">Kapsalons</a></div>
        ...
        <div className="modal-overlay" onClick={e => e.target===e.currentTarget && closeModal()}>
          <div className="modal">
            <div className="modal-header">
              <div>
                <h2 id="mname">{modal.salon.label}</h2>
                <p>📍 {modal.salon.location}</p>
              </div>
            ...
            <div className="modal-body">
                  ...
                  {selDate ? (
                    <>
                      ...
                      <div className="time-slots">
                        {morningSlots.map(s => { const taken=isTaken(selDate,s); return <div key={s} className={`time-slot ${taken?'taken':selTime===s?'selected':''}`} onClick={() => !taken&&setSelTime(s)}>{s}</div> })}
                      </div>
                      <div className="slots-lbl">☀️ Middag</div>
                      <div className="time-slots">
                        {afternoonSlots.map(s => { const taken=isTaken(selDate,s); return <div key={s} className={`time-slot ${taken?'taken':selTime===s?'selected':''}`} onClick={() => !taken&&setSelTime(s)}>{s}</div> })}
                      </div>
                    </>
              ...
              {step===4 && selSvc && selDate && (
                <div className="confirm-sum">
                  {[['Salon',modal.salon.label],['Dienst',`${selSvc.name} (${selSvc.dur})`],['Datum',formatDate(selDate)],['Tijdslot',selTime],['Huisdier',`${petName} (${petBreed})`],['Eigenaar',ownerName],['Contact',ownerPhone],['Totaal',`€${selSvc.price}`]].map(([lbl,val]) => (
              ...
              {step===5 && selSvc && selDate && (
                <div className="success-state">
                  ...
                  <div className="success-details">
                    <strong>📍 {modal.salon.label}</strong><br/>
                    {modal.salon.location}<br/><br/>
                    ...
                    📧 Bevestiging verstuurd naar {ownerEmail}
                  </div>
