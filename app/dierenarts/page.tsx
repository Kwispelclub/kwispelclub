...
export default function DierenartsPagina() {
  ...
  const mapSrc = `https://www.google.com/maps/embed/v1/search?key=AIzaSyB-mTCsAuoGU76UVEM-mDz2BKmB5pHUL7A&q=dierenarts+${encodeURIComponent(zoekStad)}&language=nl`
  ...
  const CSS = `
    ...
    *{margin:0;padding:0;box-sizing:border-box}body{font-family:'Nunito',sans-serif;background:var(--cream);color:var(--text-dark);overflow-x:hidden;-webkit-font-smoothing:antialiased}h1,h2,h3,h4{font-family:'Fredoka',sans-serif}
    ...
    .hero-tag{display:inline-flex;align-items:center;gap:6px;background:rgba(255,255,255,.15);padding:6px 16px;border-radius:50px;color:rgba(255,255,255,.9);font-size:12px;font-weight:700;margin-bottom:20px}
    ...
    .search-box button{padding:16px 28px;border-radius:50px;background:var(--orange-main);color:white;border:none;font-family:'Fredoka',sans-serif;font-size:15px;font-weight:600;cursor:pointer;transition:all .2s;white-space:nowrap;box-shadow:0 4px 16px rgba(232,145,58,.4)}
    ...
    .spoed-inner{background:linear-gradient(135deg,#922b21,#c0392b);border-radius:16px;padding:16px 24px;display:flex;align-items:center;gap:16px;color:white}
    ...
    .links-section{background:var(--green-pale);border-radius:28px;padding:40px 48px;margin:0 clamp(16px,4vw,48px) 56px;max-width:1320px;margin-left:auto;margin-right:auto}
    ...
    .link-card:hover .link-icon{background:var(--green-main)}
    ...
    @media(max-width:768px){.tips-grid{grid-template-columns:1fr}.links-grid{grid-template-columns:1fr}.search-box{flex-direction:column}.spoed-inner{flex-wrap:wrap}.spoed-tel{margin-left:0;width:100%;text-align:center}}
  `
    ...
    <>
      ...
      <div className="spoed-banner">
        <div className="spoed-inner">
          ...
          <a href="tel:0903333" className="spoed-tel">📞 0903 333 (BE)</a>
        </div>
      </div>
