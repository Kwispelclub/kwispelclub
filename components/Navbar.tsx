...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    // Auth state — getSession is sneller dan getUser op mobiel
  ...
  const initials = (() => {
    const f = user?.user_metadata?.first_name?.[0] || ''
    const l = user?.user_metadata?.last_name?.[0] || ''
  ...
  return (
    <>
      <style>{`
        .navbar{position:sticky;top:0;z-index:100;background:rgba(255,249,240,0.92);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-bottom:1px solid rgba(0,0,0,.04);padding:0 clamp(16px,4vw,48px);transition:box-shadow .3s}
        ...
        .user-pill{display:flex;align-items:center;gap:8px;padding:6px 16px 6px 8px;border-radius:50px;background:white;border:2px solid #F5EDE0;cursor:pointer;text-decoration:none;transition:all .2s}
        .user-pill:hover{border-color:#4A7C3F}
        .user-pill .ua{width:32px;height:32px;border-radius:50%;background:#4A7C3F;color:white;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;flex-shrink:0}
        .user-pill span{font-size:13px;font-weight:700;color:#2C2C2C}
        .btn-auth{padding:9px 20px;border-radius:50px;background:#4A7C3F;color:white;font-family:'Fredoka',sans-serif;font-size:14px;font-weight:600;text-decoration:none;transition:all .2s;border:none;cursor:pointer;box-shadow:0 2px 8px rgba(74,124,63,.25)}
      ...
      <div className={`mobile-menu ${menuOpen ? 'open' : ''}`}>
        ...
        <div className="m-auth">
          {user ? (
            <>
              <a href="/account" className="btn-auth" style={{ flex: 1, textAlign: 'center' }}>
                {initials} Mijn Account
              </a>
              <button className="btn-signout" onClick={handleSignOut}>Uitloggen</button>
            </>
          ) : (
            <a href="/auth" className="btn-auth" style={{ flex: 1, textAlign: 'center' }}>
